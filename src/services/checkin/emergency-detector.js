 ...
 */
function _matchesAll(text, groups) {
...
const STROKE_KW = [
  ...
  'tai biến',
  'tai bien',
  'tay chân yếu một bên',
  'tay chan yeu mot ben',
  'không nhấc được tay',
  'khong nhac duoc tay',
...
const TRAUMA_KW = [
  ...
  'khong cu dong duoc chan',
  'không cử động được tay',
  'khong cu dong duoc tay',
  'chảy máu nhiều sau ngã',
  'chay mau nhieu sau nga',
  'tai nạn giao thông',
  'tai nan giao thong',
  'bị xe đâm',
  'bi xe dam',
...
const ABDOMINAL_PAIN_KW = ['đau bụng', 'dau bung', 'đau dạ dày', 'dau da day', 'đau vùng bụng'];
...
function detectEmergency(symptoms, profile = {}) {
  ...
  if (matchesAny(text, STROKE_KW)) {
    const _highRisk = (age !== null && age > 50) || hasHTN || hasHeart;
    return result(
      true,
      'STROKE',
      'critical',
