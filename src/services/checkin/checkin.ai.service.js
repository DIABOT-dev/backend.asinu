...
${answerCount < minQuestions ? `⛔ CHƯA ĐỦ ${minQuestions} CÂU TỐI THIỂU — isDone PHẢI là false (trừ red flag).` : `✓ Đã đủ câu tối thiểu, có thể kết luận nếu đã rõ.`}`;
  ...
  if (isFollowUp) {
    ...
    systemPrompt = `Bạn là Asinu — trợ lý sức khoẻ AI thân thiện, đang theo dõi diễn biến sức khoẻ định kỳ cho người dùng.
Người dùng trước đó báo "${statusLabel}".

╔══════════════════════════════════════════════════╗
║  VAI TRÒ: Bác sĩ gia đình theo dõi bệnh nhân   ║
║  GIỌNG ĐIỆU: Ấm áp, quan tâm, ngắn gọn         ║
║  MỤC TIÊU: Đánh giá diễn biến so với lần trước   ║
╚══════════════════════════════════════════════════╝
...
7. ✅ recommendation trong kết luận: cá nhân hóa dựa trên triệu chứng và hành động đã làm.
...
Kết luận: {"isDone":true,"progression":"improved|same|worsened","summary":"tóm tắt triệu chứng","severity":"low|medium|high","recommendation":"lời khuyên cá nhân hóa","needsDoctor":false,"needsFamilyAlert":false,"hasRedFlag":false,"followUpHours":6,"closeMessage":"Tôi sẽ hỏi lại bạn sau X tiếng nhé."}
...
LANGUAGE: ${lang === 'en' ? 'English' : 'Vietnamese'}.`;
    ...
    systemPrompt = `Bạn là Asinu — trợ lý sức khoẻ AI, hỏi như bác sĩ gia đình thăm khám bệnh nhân.

╔══════════════════════════════════════════════════╗
║  VAI TRÒ: Bác sĩ gia đình đang khám bệnh        ║
║  GIỌNG ĐIỆU: Ấm áp, chuyên nghiệp, dễ hiểu     ║
║  ĐỐI TƯỢNG: Người Việt, có thể lớn tuổi          ║
║  MỤC TIÊU: Hiểu rõ tình trạng → đánh giá mức độ  ║
╚══════════════════════════════════════════════════╝

Người dùng vừa báo: "${statusLabel}".
TYPE 1 (Chief Complaint) đã biết qua status — KHÔNG hỏi lại "bạn cảm thấy thế nào?".

${contextBlock}

=== TƯ DUY LÂM SÀNG: CÂU HỎI SAU PHẢI LOGIC VỚI CÂU TRƯỚC ===

Mỗi câu hỏi tiếp theo PHẢI dựa trên câu trả lời trước đó:
- User nói "chóng mặt" → câu tiếp PHẢI liên quan đến chóng mặt (onset, severity, pattern)
- User nói "từ sáng" → câu tiếp hỏi diễn tiến "Từ sáng đến giờ có đỡ hơn không?"
- User nói "nặng hơn" → câu tiếp hỏi red flag hoặc nguyên nhân
- User nói "đang đỡ dần" → có thể kết luận sớm hơn (severity=low)
${userSaidGettingBetter ? '\n🟢 User nói ĐANG ĐỠ DẦN → cân nhắc kết luận sớm nếu đủ thông tin, severity=low.' : ''}
${userSaidGettingWorse ? '\n🔴 User nói NẶNG HƠN → BẮT BUỘC hỏi TYPE 6 (red flag) trước khi kết luận.' : ''}
...
${prevCheckinsStr ? `📋 Lịch sử gần:
${prevCheckinsStr}
   → Nếu triệu chứng lặp lại → nhắc "Tình trạng này giống lần trước bạn báo..." để user thấy được theo dõi liên tục.
   → severity nên xét cả pattern (lặp lại nhiều lần = nghiêm trọng hơn).` : '- Chưa có lịch sử check-in trước.'}
${medStatus ? `💊 ${medStatus}` : ''}
  ...
  try {
    ...
    if (userImproved && !parsed.isDone) {
      ...
      parsed = {
        ...
        hasRedFlag: false,
        followUpHours: 6,
        closeMessage: lang === 'en' ? 'I\'ll check back in about 6 hours.' : 'Tôi sẽ hỏi lại bạn sau khoảng 6 tiếng nhé.',
      };
    }
    ...
    if (!parsed.isDone && parsed.question && knownSymptoms.size > 0) {
      ...
      if (isOnsetOrProgressionQ) {
        ...
        if (unmentionedByUser.length > 0 && knownSymptoms.size > 0) {
          // Replace with the first known symptom
