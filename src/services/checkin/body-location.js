...
const BODY_LOCATIONS = ['head', 'chest', 'abdomen', 'limbs', 'skin', 'whole_body', 'mental'];
...
const EMERGENCY_KEYWORDS_BY_LOCATION = {
  head: [
    'đau đầu dữ dội',
    'thunderclap',
    'sét đánh', // Sudden severe → SAH
    'yếu liệt nửa người',
    'méo miệng',
    'nói ngọng', // Stroke FAST
    'mất thị lực',
    'mất ý thức',
    'co giật',
  ],
  chest: [
    ...
    'khó thở dữ dội',
    'tím tái',
    'không nói được', // Severe respiratory
    'ngất',
    'tim đập rất nhanh + ngất', // Arrhythmia + syncope
  ],
  abdomen: [
    'đau bụng dữ dội',
    'đau bụng + nôn ra máu',
    ...
    'phân đen', // GI bleed
    'cứng bụng',
    'đau bụng đột ngột', // Acute abdomen
  ],
  limbs: [
    'yếu liệt đột ngột',
    'tê hoàn toàn nửa người', // Stroke
    'sưng đau bắp chân + khó thở', // PE / DVT
  ],
  skin: [
    'mề đay + khó thở',
    'phù mặt + khó thở', // Anaphylaxis
    'vàng da đột ngột',
    'xuất huyết dưới da', // Acute liver / coag
  ],
  whole_body: [
    'sốt cao + cứng cổ',
    'sốt + co giật', // Meningitis
    'lú lẫn + sốt',
    'mất ý thức',
    'shock',
    'sốc',
  ...
  mental: [
    'muốn tự tử',
    'tự sát',
    'tự hại', // Suicide ideation
    'nghe tiếng nói',
    'ảo giác cấp', // Acute psychosis
  ],
};
...
function buildLocationContext(locations = [], other = null, lang = 'vi') {
  ...
  if (named.length === 0) parts.push('Chưa rõ vùng khó chịu');
    ...
    parts.push(`Vùng khó chịu: ${named.join(', ')} → hỏi cross-correlation giữa các vùng.`);
  else
    parts.push(
      `Khó chịu nhiều vùng (${named.length}: ${named.join(', ')}) → hỏi systemic + assess toàn thân.`
    );
