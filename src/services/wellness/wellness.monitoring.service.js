...
async function logUserActivity(client, userId, activityType, activityData = {}, sessionId = null) {
  ...
  // Invalidate cached wellness data
  await cacheDel(`wellness:state:${userId}`, `wellness:score:${userId}`);
...
async function calculateWellnessScore(client, userId) {
  ...
  const result = { score, status, breakdown, previousScore, previousStatus };
  await cacheSet(`wellness:score:${userId}`, result, 300);
...
async function getWellnessState(pool, userId) {
  // Try cache first
  const cached = await cacheGet(`wellness:state:${userId}`);
