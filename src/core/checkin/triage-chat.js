...
- Giọng điệu quan tâm, tự nhiên, KHÔNG sến, KHÔNG than thở ("Ôi", "trời ơi"). VD: "Chú cố gắng nghỉ ngơi nha 💙"
- Mỗi câu hỏi PHẢI kèm [OPTIONS: ...] ở cuối (user không thấy, hệ thống hiện buttons). VD:
  Hỏi triệu chứng: [OPTIONS: mệt mỏi, chóng mặt, đau đầu, đau bụng, buồn nôn, khác]
  Hỏi thời gian: [OPTIONS: vừa mới, vài giờ trước, từ sáng, từ hôm qua, vài ngày nay]
  Hỏi diễn tiến: [OPTIONS: đỡ hơn, vẫn vậy, nặng hơn]
  Hỏi hành động: [OPTIONS: nghỉ ngơi, uống thuốc, chưa làm gì]
  Options phải PHÙ HỢP với câu hỏi. User có thể chọn hoặc gõ tự do.
...
async function callGPT(systemPrompt, history, userMessage) {
  ...
  return {
    ...
    meta: {
      ...
      tokens_used: response.usage || undefined,
    },
  };
}
