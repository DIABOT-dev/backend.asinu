...
const QUESTION_REWRITES = {
  ...
  slider_pain: {
    ...
    match: (text) =>
      text.includes('đau mức') || text.includes('mức độ đau') || text.includes('pain level'),
  ...
  duration: {
    id: 'rewrite_duration',
    match: (text) =>
      text.includes('từ khi nào') ||
      text.includes('bao lâu') ||
      text.includes('how long') ||
      text.includes('since when'),
...
function selectContinuityPrefix(ctx, user) {
  ...
  if (symptom && ctx.consecutiveTiredDays >= 3) {
    return {
      text: _renderTemplate(
        CONTINUITY_PREFIXES.same_symptom_3d,
        { symptom, days: ctx.consecutiveTiredDays },
        h,
        lang
      ),
...
const EMPATHY_RESPONSES = {
  ...
  answer_mild: {
    ...
    en: [
      "I see, let's keep monitoring.",
      'Understood, let me ask a bit more.',
      "Okay, let's check a few more things.",
    ],
  },
  answer_severe: {
    id: 'empathy_severe',
    vi: [
      '{selfRef} sẽ theo sát thêm nhé.',
      'Để {selfRef} hỏi kỹ hơn.',
      '{selfRef} hơi lo cho {honorific}.',
    ],
    en: [
      'Let me ask a few more questions.',
      "I'd like to check more carefully.",
      "I'm a bit concerned, let me ask more.",
    ],
  },
  answer_improving: {
  ...
  answer_worsening: {
    ...
    vi: [
      '{selfRef} hơi lo cho {honorific}.',
      'Mình cần theo dõi kỹ hơn.',
      '{selfRef} muốn nắm rõ thêm tình hình.',
    ],
...
function selectEmpathyResponse(lastAnswer, user) {
  ...
  const idx =
    Math.abs(answerStr.split('').reduce((a, c) => a + c.charCodeAt(0), 0)) % variants.length;
...
async function buildCheckinContext(pool, userId) {
  ...
  const consecutiveTiredDays = recentCheckins.filter(
    (c) => c.initial_status === 'tired' || c.initial_status === 'very_tired'
  ).length;
 ...
 */
function validateOutput(rewritten, _originalQuestion) {
  const errors = [];
...
function validateScriptIntegrity(originalScriptData, illusionOutput) {
  ...
  if (illusionOutput.totalSteps !== undefined && originalScriptData.questions) {
    if (illusionOutput.totalSteps !== originalScriptData.questions.length) {
      errors.push(
        `totalSteps changed: ${originalScriptData.questions.length} → ${illusionOutput.totalSteps}`
      );
    }
  }
