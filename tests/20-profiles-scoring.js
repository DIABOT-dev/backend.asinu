...
const PROFILES = [
  ...
  {
    id: 6,
    label: 'Female 45 diabetes+hypertension',
    gender: 'nữ',
    age: 45,
  ...
  {
    id: 8,
    label: 'Female 55 diabetes+heart',
    gender: 'nữ',
    age: 55,
    conditions: ['tiểu đường', 'tim mạch'],
  },
  ...
  {
    id: 12,
    label: 'Female 68 diabetes+hypertension',
    gender: 'nữ',
    age: 68,
    conditions: ['tiểu đường', 'huyết áp cao'],
  },
  {
    id: 13,
    label: 'Male 70 diabetes+hyp+heart',
    gender: 'nam',
    age: 70,
    conditions: ['tiểu đường', 'huyết áp cao', 'tim mạch'],
  },
  {
    id: 14,
    label: 'Female 75 4-conditions',
    gender: 'nữ',
    age: 75,
    conditions: ['tiểu đường', 'huyết áp cao', 'tim mạch', 'phổi tắc nghẽn'],
  },
  { id: 15, label: 'Male 80 diabetes', gender: 'nam', age: 80, conditions: ['tiểu đường'] },
  {
    id: 16,
    label: 'Female 85 diabetes+heart+hyp+kidney',
    gender: 'nữ',
    age: 85,
    conditions: ['tiểu đường', 'tim mạch', 'huyết áp cao', 'thận'],
  },
  {
    id: 17,
    label: 'Male 30 diabetes (young+diabetes)',
    gender: 'nam',
    age: 30,
...
function isElderly(p) {
  return p.age >= 60;
}
function hasConditions(p) {
  return p.conditions && p.conditions.length > 0;
}
function hasCondition(p, name) {
  return p.conditions && p.conditions.some((c) => c.includes(name));
}
...
async function main() {
  ...
  for (let i = 0; i < scriptData.scoring_rules.length; i++) {
    ...
    const conds = r.conditions
      ...
      .join(' & ');
    console.log(
      `  Rule ${i}: ${conds || '(empty)'} → ${r.severity} | doctor=${r.needs_doctor} | family=${r.needs_family_alert}`
    );
  }
  console.log();
  ...
  if (scriptData.condition_modifiers) {
    ...
    for (const m of scriptData.condition_modifiers) {
      const extra = m.extra_conditions
        ? m.extra_conditions.map((c) => `${c.field} ${c.op} ${c.value}`).join(' & ')
        : 'none';
  ...
  for (const profile of PROFILES) {
    for (const [setKey, answerSet] of Object.entries(ANSWER_SETS)) {
      ...
      try {
        ...
        if (
          !isElderly(profile) &&
          !hasConditions(profile) &&
          setKey === 'mild' &&
          profile.id !== 20
        ) {
          if (scoring.severity !== 'low')
            errs.push(`Expected LOW for young healthy + mild, got ${scoring.severity}`);
        }
        ...
        if (
          ...
          if (scoring.severity !== 'high')
            errs.push(`Expected HIGH for young healthy + severe, got ${scoring.severity}`);
        }
        ...
        if (isElderly(profile) && hasConditions(profile) && setKey === 'mild') {
          if (hasCondition(profile, 'tim')) {
            ...
            if (scoring.severity !== 'high')
              errs.push(
                `Expected HIGH for elderly+heart+mild (unconditional modifier), got ${scoring.severity}`
              );
          } else {
            if (scoring.severity !== 'medium')
              errs.push(`Expected MEDIUM for elderly+conditions+mild, got ${scoring.severity}`);
            if (scoring.severity === 'high')
              errs.push(
                `Should NOT be HIGH for mild symptoms even with elderly+conditions (no heart)`
              );
          }
        }
        ...
        if (isElderly(profile) && hasConditions(profile) && setKey === 'severe') {
          if (scoring.severity !== 'high')
            errs.push(`Expected HIGH for elderly+conditions+severe, got ${scoring.severity}`);
        }
        ...
        if (scoring.needsFamilyAlert && scoring.severity !== 'critical') {
          errs.push(
            `needsFamilyAlert should be false for first-time check-in (severity=${scoring.severity})`
          );
        }
        ...
        if (errs.length > 0) {
          ...
          console.log(
            `  FAIL  ${testLabel} → ${scoring.severity.toUpperCase().padEnd(8)} doctor=${String(scoring.needsDoctor).padEnd(5)} family=${String(scoring.needsFamilyAlert).padEnd(5)} mods=${scoring.modifiersApplied.join(', ') || 'none'}`
          );
          for (const e of errs) console.log(`        *** ${e}`);
        } else {
          passed++;
          console.log(
            `  PASS  ${testLabel} → ${scoring.severity.toUpperCase().padEnd(8)} doctor=${String(scoring.needsDoctor).padEnd(5)} family=${String(scoring.needsFamilyAlert).padEnd(5)} mods=${scoring.modifiersApplied.join(', ') || 'none'}`
          );
        }
      } catch (err) {
        failed++;
  ...
  console.log('-'.repeat(116));
  console.log(
    `Legend: +doc = needsDoctor  |  +fam = needsFamilyAlert  |  * = condition modifier applied`
  );
  ...
  const diabetesMedium = results.filter(
    (r) =>
  ...
  console.log(
    `  Diabetes + MEDIUM answers: ${diabetesModified.length}/${diabetesMedium.length} had modifiers applied`
  );
  ...
  console.log(
    `  Heart disease + ANY answers: ${heartModified.length}/${heartAny.length} had modifiers applied`
  );

  // Check elderly bump
  const elderlyMild = results.filter(
    (r) =>
  ...
  console.log(
    `  Elderly+conditions + MILD: ${elderlyBumped.length}/${elderlyMild.length} bumped from LOW`
  );
  ...
  console.log(
    `  Engine-Runner agreement: ${mismatches.length === 0 ? 'ALL MATCH' : `${mismatches.length} MISMATCHES`}`
  );
