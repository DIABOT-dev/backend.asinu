...
(async () => {
  try {
    ...
    TEST_USER_ID =
      users.find((u) => u.id === 3)?.id || users.find((u) => u.id === 4)?.id || users[0]?.id;
    ...
    assert(
      'A6: random string -> slugified (no crash)',
    ...
    assert(
      'A7: empty string -> handled (no crash)',
      typeof emptyKey === 'string',
      `got "${emptyKey}"`
    );

    const nullKey = toClusterKey(null);
    assert('A8: null -> handled (no crash)', typeof nullKey === 'string', `got "${nullKey}"`);
    ...
    assert('B1: returns 3 clusters', clusters.length === 3, `got ${clusters.length}`);
    ...
    assert('B2: 3 rows in problem_clusters', dbClusters.length === 3, `got ${dbClusters.length}`);
    ...
    assert(
      'B3: first symptom has highest priority',
    ...
    assert(
      'B5: scripts generated (triage_scripts rows > 0)',
      scripts.length > 0,
    ...
    assert(
      'B6: initial scripts exist for each cluster',
      initialScripts.length >= 3,
      `got ${initialScripts.length} initial scripts`
    );
    assert(
      'B7: followup scripts exist for each cluster',
    ...
    assert(
      'B8: duplicate call -> no extra clusters (ON CONFLICT)',
    ...
    const { rows: afterOverlap } = await pool.query(
      'SELECT * FROM problem_clusters WHERE user_id = $1',
      [TEST_USER_ID]
    );
    const hasAbdominal = afterOverlap.some((c) => c.cluster_key === 'abdominal_pain');
    assert(
      'B9: overlapping call -> adds new cluster',
      hasAbdominal,
      `clusters: ${afterOverlap.map((c) => c.cluster_key).join(', ')}`
    );
    assert(
      'B10: overlapping call -> existing cluster updated (not duplicated)',
    ...
    if (otherUsers.length > 0) {
      ...
      assert(
        'C6: user with no clusters -> returns null',
        emptyResult === null,
    ...
    assert(
      'D1: get headache initial script -> non-null',
      headacheScript !== null,
    ...
    if (headacheScript) {
      ...
      assert(
        'D2: script_data has questions',
        sd && Array.isArray(sd.questions) && sd.questions.length > 0,
        `questions count: ${sd?.questions?.length}`
      );
      assert(
        'D3: script_data has scoring_rules',
        sd && Array.isArray(sd.scoring_rules) && sd.scoring_rules.length > 0,
        `scoring_rules count: ${sd?.scoring_rules?.length}`
      );
      assert(
        'D4: script_data has conclusion_templates',
        sd &&
          sd.conclusion_templates &&
          sd.conclusion_templates.low &&
          sd.conclusion_templates.medium &&
          sd.conclusion_templates.high,
    ...
    assert('D5: non-existent cluster -> returns null', noScript === null, `got ${noScript}`);

    // Followup script
    const followupScript = await getScript(pool, TEST_USER_ID, 'headache', 'followup');
    assert(
      'D6: followup script type filtering',
    ...
    const newCluster = await addCluster(
      pool,
      TEST_USER_ID,
      'gastric_pain',
      'đau dạ dày',
    ...
    assert(
      'E1: addCluster returns cluster object',
    ...
    assert(
      'E2: gastric_pain in problem_clusters',
      pcCheck.length === 1,
    ...
    assert(
      'E3: script auto-generated for gastric_pain',
      gastricScript !== null,
      `got ${gastricScript ? 'script' : 'null'}`
    );

    // Add same cluster again -> no duplicate
    const dupCluster = await addCluster(
      pool,
      TEST_USER_ID,
      'gastric_pain',
      'đau dạ dày',
    ...
    assert(
      'E4: duplicate addCluster -> no duplicate row',
      pcDupCheck.length === 1,
    ...
    assert('F1: count_7d updated', statsCheck[0]?.count_7d === 5, `got ${statsCheck[0]?.count_7d}`);
    assert(
      'F2: count_30d updated',
    ...
    if (sd) {
      ...
      assert(
        'G1: all questions have id, text, type',
        allQsValid,
      ...
      // scoring_rules array
      assert(
        'G2: scoring_rules is non-empty array',
      ...
      assert(
        'G3: conclusion_templates has low/medium/high',
        sd.conclusion_templates?.low &&
          sd.conclusion_templates?.medium &&
          sd.conclusion_templates?.high,
      ...
      if (
        headacheMapping &&
        headacheMapping.data.followUpQuestions &&
        headacheMapping.data.followUpQuestions.length > 0
      ) {
        ...
        assert(
          'G6: questions use clinical-mapping followUpQuestions',
          scriptUsesMapping,
          `mapping Q1="${mappingQ1}", script Qs: ${sd.questions.map((q) => q.text.substring(0, 30)).join(' | ')}`
        );
      } else {
        assert(
          'G6: headache has no followUpQuestions in mapping (skip)',
          true,
          'no mapping followUpQuestions'
        );
      }
    } else {
      for (let i = 1; i <= 6; i++) {
    ...
    try {
      ...
      assert('H1: user with/without onboarding profile -> no crash', true, 'no crash');
    } catch (e) {
      assert('H1: user with/without onboarding profile -> no crash', false, e.message);
    }

    // H2: empty symptoms array -> no clusters created
    const beforeEmpty = (
      await pool.query('SELECT count(*) FROM problem_clusters WHERE user_id = $1', [TEST_USER_ID])
    ).rows[0].count;
    ...
    assert(
      'H2: empty symptoms array -> no new clusters',
    ...
    try {
      ...
      assert(
        'H3: very long symptom name -> handled (no crash)',
        typeof longKey === 'string',
        `key length: ${longKey.length}`
      );
    } catch (e) {
      assert('H3: very long symptom name -> handled (no crash)', false, e.message);
    }
    ...
    assert(
      'H4: Vietnamese diacritics preserved in display_name',
