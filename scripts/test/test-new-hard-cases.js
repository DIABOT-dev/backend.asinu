...
const {
  ...
  parseSliderFromText,
  localMatch,
} = require('../../src/core/agent/ai-answer-parser');
const {
  ...
  return {
    ...
    questions: [
      ...
      },
      {
        ...
        type: 'single_choice',
        options: [
          'Binh thuong, van lam viec duoc',
          'Kho chiu nhung chiu duoc',
          'Nang, phai nam nghi',
        ],
      },
    ],
    scoring_rules: [
      ...
      {
        conditions: [{ field: 'q_severity', op: 'gte', value: 7 }],
        combine: 'and',
        severity: 'high',
        follow_up_hours: 1,
        needs_doctor: true,
      ...
      {
        conditions: [{ field: 'q_severity', op: 'gte', value: 4 }],
        combine: 'and',
        severity: 'medium',
        follow_up_hours: 3,
        needs_doctor: false,
      ...
      {
        conditions: [{ field: 'q_severity', op: 'lt', value: 4 }],
        combine: 'and',
        severity: 'low',
        follow_up_hours: 6,
        needs_doctor: false,
    ...
    conclusion_templates: {
      low: {
        summary: 'Trieu chung nhe.',
        recommendation: 'Nghi ngoi.',
        close_message: 'Theo doi them.',
      },
      medium: {
        summary: 'Trieu chung vua.',
        recommendation: 'Theo doi.',
        close_message: 'Se hoi lai.',
      },
      high: {
        summary: 'Trieu chung nang.',
        recommendation: 'Di kham.',
        close_message: 'Di kham som.',
      },
    ...
    followup_questions: [
      {
        id: 'fu1',
        text: 'So voi luc truoc?',
        type: 'single_choice',
        options: ['Do hon', 'Van vay', 'Nang hon'],
      },
...
async function testGroupB() {
  ...
  for (const t of sliderTests) {
    try {
      ...
      record(
        ...
        t.input,
        'correct range',
        `value=${result.value}, method=${result.method}, conf=${result.confidence}`,
        pass
      );
    } catch (err) {
  ...
  for (const t of choiceTests) {
    try {
      ...
      record(
        ...
        t.input,
        'correct match',
        `matched=${JSON.stringify(result.matched)}, method=${result.method}`,
        pass
      );
    } catch (err) {
      record('B', t.name, t.input, 'no crash', `CRASH: ${err.message}`, false);
  ...
  try {
    ...
    record(
      'B',
      'B6: multi "vua oi vua chong mat + nhin mo" -> multiple',
      multiInput,
      '>=1 match',
  ...
  try {
    ...
    record(
      'B',
      'B12: negation "dau dau + buon non, hong co sot"',
      negInput,
      'dau dau+buon non, not sot',
      `matched=${JSON.stringify(result.matched)}`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'B',
      'B15: ordinal "cai thu hai tu tren xuong" -> graceful',
      ordinalInput,
      'no crash/graceful',
      `matched=${JSON.stringify(result.matched)}, method=${result.method}`,
      pass
...
async function testGroupC() {
  ...
  try {
    ...
    while (!step.isDone && iterations < 20) {
      ...
      let ans;
      if (q.type === 'slider')
        ans = 5; // fallback numeric for slider
      else if (q.options)
        ans = q.options[0]; // must give valid option
      else ans = 'khong biet';
    ...
    record(
      'C',
      'C1: All "khong biet" -> still concludes',
      'khong biet x N',
      'conclusion exists',
      `isDone=${step.isDone}, severity=${step.conclusion?.severity}`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'C',
      'C3: "tam" -> 8 via parseSliderFromText',
      'tam',
      'value=8',
      `value=${sliderResult.value}, method=${sliderResult.method}`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'C',
      'C4: Duplicate question answer -> no crash',
      'q_severity x2',
      'no crash',
      `isDone=${step.isDone}, step=${step.currentStep}`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'C',
      'C5: 500+ char answer -> scores ok',
      `${longAnswer.length} chars`,
      'valid severity',
  ...
  try {
    ...
    record(
      'C',
      'C8: "pain nhieu lam" mixed lang -> handled',
      mixInput,
      'no crash',
      `matched=${JSON.stringify(result.matched)}, method=${result.method}`,
      pass
  ...
  try {
    const result = localMatch(
      '3',
      ['Buon non', 'Chong mat', 'Mo mat', 'Khong co'],
      'single_choice'
    );
    const pass = true; // no crash
    record(
      'C',
      'C9: "3" for choice -> handled',
      '3',
      'no crash',
      `matched=${JSON.stringify(result.matched)}, method=${result.method}`,
      pass
...
async function testGroupD() {
  ...
  try {
    ...
    const parsedImpact = localMatch(
      'nang lam nam liet luon',
      scriptData.questions[5].options,
      'single_choice'
    );
    ...
    record(
      'D',
      'D1: Direct vs parsed impact -> same severity',
      'direct vs parsed',
      `same severity`,
      `direct=${direct.severity}, parsed=${parsed.severity}, parsedTo=${parsedVal}`,
      pass
    );
  } catch (err) {
    record(
      'D',
      'D1: Direct vs parsed impact',
      'compare',
      'no crash',
      `CRASH: ${err.message}`,
      false
  ...
  try {
    ...
    record(
      'D',
      'D2: Slider 8 vs "tam diem" -> same severity',
      '8 vs tam',
      `same severity`,
      `direct=${directResult.severity}, parsed=${parsedResult.severity}, sliderVal=${parsedSlider.value}`,
      pass
    );
  } catch (err) {
    record(
      'D',
      'D2: Slider direct vs parsed',
      '8 vs tam',
      'no crash',
      `CRASH: ${err.message}`,
      false
  ...
  try {
    ...
    record(
      'D',
      'D3: Slider 3 vs "ba diem" -> same severity',
      '3 vs ba',
      `same severity`,
      `direct=${directResult.severity}, parsed=${parsedResult.severity}, sliderVal=${parsedSlider.value}`,
      pass
  ...
  try {
    ...
    record(
      'D',
      'D4: "kieu nhoi nhoi" -> "Dau nhoi tung con"',
      'kieu nhoi nhoi',
      'correct match',
  ...
  try {
    ...
    const parsedAccompany = localMatch(
      'nhin mo mo',
      scriptData.questions[3].options,
      'multi_choice'
    );
    const hasMoMat =
      parsedAccompany.matched &&
      parsedAccompany.matched.some(
        (m) => m.includes('Mo mat') || m.includes('mo mat') || m === 'Mo mat'
      );
    ...
    if (hasMoMat) {
      ...
      record(
        'D',
        'D6: "nhin mo mo" -> Mo mat -> same danger',
        'nhin mo mo',
        `same severity as direct`,
        `direct=${directResult.severity}, parsed=${parsedResult.severity}`,
        pass
      );
    } else {
      record(
        ...
        'nhin mo mo',
        'should parse Mo mat',
        `parsed=${JSON.stringify(parsedAccompany.matched)}`,
        false
      );
    }
  } catch (err) {
  ...
  try {
    ...
    record(
      'D',
      'D7: Elderly + conditions + low answers -> bumped',
      'severity=2',
      'not low',
      `severity=${result.severity}, mods=${JSON.stringify(result.modifiersApplied)}`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'D',
      'D8: Diabetes + severity 5 -> bumped to high+',
      'severity=5+diabetes',
      'high or critical',
      `severity=${result.severity}, mods=${JSON.stringify(result.modifiersApplied)}`,
      pass
  ...
  try {
    ...
    record(
      'D',
      'D9: Follow-up "Do hon" vs "Nang hon"',
      'better vs worse',
      'worse >= better severity',
      `better=${betterResult.severity}, worse=${worseResult.severity}`,
      pass
    );
  } catch (err) {
    record(
      'D',
      'D9: Follow-up scoring',
      'better vs worse',
      'no crash',
      `CRASH: ${err.message}`,
      false
  ...
  try {
    ...
    record(
      'D',
      'D10: Full script -> valid conclusion',
      'full run',
      'valid conclusion',
      `severity=${step.conclusion?.severity}, needsDoctor=${step.conclusion?.needsDoctor}`,
      pass
    );
  } catch (err) {
...
async function testGroupE() {
  ...
  try {
    ...
    for (let i = 0; i < 50; i++) {
      inputs.push(
        ['tam', 'ba', 'nam', 'muoi', 'khong', 'dau lam', 'nhe thoi', 'nang qua', '5', '3'][i % 10]
      );
    }
    ...
    record(
      'E',
      'E1: parseSliderFromText 50x rapid',
      '50 calls',
      `all valid + consistent, <1s`,
      `valid=${allValid}, consistent=${consistent}, elapsed=${elapsed}ms`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'E',
      'E2: evaluateScript 100x random',
      '100 calls',
      `all valid severities, <1s`,
      `allValid=${allValid}, elapsed=${elapsed}ms, sample=${severities.slice(0, 5).join(',')}`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'E',
      'E3: matchCluster 100x concurrent',
      '100 calls',
      `all valid + consistent`,
      `allValid=${allValid}, consistent=${consistent}, elapsed=${elapsed}ms`,
      pass
    );
  } catch (err) {
  ...
  try {
    ...
    record(
      'E',
      'E4: getNextQuestion 50 full sessions',
      '50 sessions',
      `all 50 complete`,
      `completed=${completedSessions}/50, elapsed=${elapsed}ms`,
      pass
    );
  } catch (err) {
