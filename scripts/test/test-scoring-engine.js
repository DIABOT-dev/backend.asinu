...
assert(
  'eq: 5 eq 5 -> true',
  evaluateCondition({ field: 'q1', op: 'eq', value: 5 }, mkMap({ q1: 5 })),
  true
);
assert(
  'eq: 5 eq 6 -> false',
  evaluateCondition({ field: 'q1', op: 'eq', value: 6 }, mkMap({ q1: 5 })),
  false
);
assert(
  'neq: 5 neq 6 -> true',
  evaluateCondition({ field: 'q1', op: 'neq', value: 6 }, mkMap({ q1: 5 })),
  true
);
assert(
  'gt: 6 gt 5 -> true',
  evaluateCondition({ field: 'q1', op: 'gt', value: 5 }, mkMap({ q1: 6 })),
  true
);
assert(
  'gt: 5 gt 5 -> false',
  evaluateCondition({ field: 'q1', op: 'gt', value: 5 }, mkMap({ q1: 5 })),
  false
);
assert(
  'gte: 5 gte 5 -> true',
  evaluateCondition({ field: 'q1', op: 'gte', value: 5 }, mkMap({ q1: 5 })),
  true
);
assert(
  'gte: 4 gte 5 -> false',
  evaluateCondition({ field: 'q1', op: 'gte', value: 5 }, mkMap({ q1: 4 })),
  false
);
assert(
  'lt: 4 lt 5 -> true',
  evaluateCondition({ field: 'q1', op: 'lt', value: 5 }, mkMap({ q1: 4 })),
  true
);
assert(
  'lt: 5 lt 5 -> false',
  evaluateCondition({ field: 'q1', op: 'lt', value: 5 }, mkMap({ q1: 5 })),
  false
);
assert(
  'lte: 5 lte 5 -> true',
  evaluateCondition({ field: 'q1', op: 'lte', value: 5 }, mkMap({ q1: 5 })),
  true
);
assert(
  'lte: 6 lte 5 -> false',
  evaluateCondition({ field: 'q1', op: 'lte', value: 5 }, mkMap({ q1: 6 })),
  false
);
assert(
  'contains: "dau dau du doi" contains "dau dau" -> true',
  evaluateCondition(
    { field: 'q1', op: 'contains', value: 'đau đầu' },
    mkMap({ q1: 'đau đầu dữ dội' })
  ),
  true
);
assert(
  'contains: "met" contains "dau" -> false',
  evaluateCondition({ field: 'q1', op: 'contains', value: 'đau' }, mkMap({ q1: 'mệt' })),
  false
);
assert(
  'in: "a" in ["a","b"] -> true',
  evaluateCondition({ field: 'q1', op: 'in', value: ['a', 'b'] }, mkMap({ q1: 'a' })),
  true
);
assert(
  'in: "c" in ["a","b"] -> false',
  evaluateCondition({ field: 'q1', op: 'in', value: ['a', 'b'] }, mkMap({ q1: 'c' })),
  false
);
assert(
  'not_in: "c" not_in ["a","b"] -> true',
  evaluateCondition({ field: 'q1', op: 'not_in', value: ['a', 'b'] }, mkMap({ q1: 'c' })),
  true
);
assert(
  'unknown operator -> false',
  evaluateCondition({ field: 'q1', op: 'banana', value: 5 }, mkMap({ q1: 5 })),
  false
);
assert(
  'missing field -> false',
  evaluateCondition({ field: 'q_missing', op: 'eq', value: 5 }, mkMap({ q1: 5 })),
  false
);
...
assert(
  'AND with 2 true conditions -> true',
  evaluateRule(
    {
      conditions: [
...
assert(
  'AND with 1 false condition -> false',
  evaluateRule(
    {
      conditions: [
        { field: 'q1', op: 'gte', value: 5 },
...
assert('Empty conditions -> false', evaluateRule({ conditions: [] }, mkMap({ q1: 5 })), false);
...
res = evaluateScript(
  { scoring_rules: [{ severity: 'high', conditions: [{ field: 'q1', op: 'eq', value: 999 }] }] },
