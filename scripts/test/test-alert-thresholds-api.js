...
function pickMedium(q) {
  ...
  if (q.type === 'multi_choice') {
    ...
    const safe = opts.find(
      (o) =>
        !o.includes('không có') && !o.includes('mờ') && !o.includes('cứng') && !o.includes('tê')
    );
...
async function runTests() {
  ...
  console.log(
    `  Clusters: ${clusterRes.ok ? clusterRes.clusters?.map((c) => c.cluster_key).join(', ') : 'FAILED: ' + JSON.stringify(clusterRes)}`
  );
  console.log('');
  ...
  try {
    ...
    assert(
      2,
      'Headache mild+không có → needsDoctor',
      'needsDoctor',
      false,
      r.conclusion?.needsDoctor,
      `severity=${r.conclusion?.severity}`
    ...
    results.push({ testId: 2, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      3,
      'Abdominal pain mildest → needsDoctor',
      'needsDoctor',
      false,
      r.conclusion?.needsDoctor,
      `severity=${r.conclusion?.severity}`
    ...
    results.push({ testId: 3, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 4, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 5, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 6, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 7, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    const r = await runFullSession('đau đầu', (q, i) => {
      ...
      if (
        ...
        q.options &&
        q.options.some((o) => o.includes('trung bình'))
      ) {
    ...
    results.push({ testId: 8, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 9, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 10, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      11,
      'Headache+mờ mắt → appropriate response',
      'needsDoctor',
      true,
      doc,
    ...
    results.push({ testId: 11, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      12,
      'Headache severe+danger → needsDoctor',
      'needsDoctor',
      true,
      r.conclusion?.needsDoctor,
      `severity=${r.conclusion?.severity}`
    ...
    results.push({ testId: 12, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    const r = await runFullSession('đau ngực', (q) => {
      ...
      if (q.type === 'multi_choice' && q.options) {
        ...
        const selected = q.options.filter(
          (o) => !o.includes('không có') && !o.includes('không rõ')
        );
    ...
    console.log(
      `  Q&A:`,
      r.questions.map((q, i) => `${q.id}(${q.type}): "${r.answers[i]?.answer}"`).join(' | ')
    );
    assert(
      13,
      'Chest pain worst → needsDoctor',
      'needsDoctor',
      true,
      r.conclusion?.needsDoctor,
    ...
    results.push({ testId: 13, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 14, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      15,
      'Fallback worst slider → needsDoctor',
      'needsDoctor',
      true,
      r.conclusion?.needsDoctor,
      `severity=${r.conclusion?.severity} matchedRule=${r.conclusion?.matchedRuleIndex}`
    ...
    results.push({ testId: 15, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      16,
      'Mild case → needsFamilyAlert',
      'needsFamilyAlert',
      false,
      r.conclusion?.needsFamilyAlert,
      `severity=${r.conclusion?.severity}`
    );
    console.log(
      `  severity=${r.conclusion?.severity} needsFamilyAlert=${r.conclusion?.needsFamilyAlert}`
    );
  } catch (e) {
    results.push({ testId: 16, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    results.push({ testId: 17, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      18,
      'First-time HIGH → needsFamilyAlert',
      'needsFamilyAlert',
      false,
      r.conclusion?.needsFamilyAlert,
    ...
    results.push({ testId: 18, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      19,
      'Headache severe 1st time → needsFamilyAlert',
      'needsFamilyAlert',
      false,
      r.conclusion?.needsFamilyAlert,
      `severity=${r.conclusion?.severity}`
    ...
    results.push({ testId: 19, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  try {
    ...
    assert(
      20,
      'Worst answers 1st session → needsFamilyAlert',
      'needsFamilyAlert',
      false,
      r.conclusion?.needsFamilyAlert,
      `severity=${r.conclusion?.severity}`
    ...
    results.push({ testId: 20, pass: false, extra: e.message });
    console.error('  ERROR:', e.message);
  }
  ...
  for (const r of results) {
    ...
    console.log(
      `  [${icon}] Test ${r.testId}: ${r.testName || 'unknown'} — ${r.field}=${r.actual} (expected ${r.expected})${detail}`
    );
  }
  ...
  if (failed.length > 0) {
    ...
    for (const r of failed) {
      console.log(
        `  Test ${r.testId}: ${r.testName} — got ${r.field}=${r.actual}, expected ${r.expected} (${r.extra})`
      );
    }
  }
