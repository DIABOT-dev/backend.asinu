...
async function sectionA() {
  ...
  test(
    'A1: Same symptoms twice -> no duplicate clusters',
  ...
  test(
    'A2: Overlapping ["dau dau","chong mat"] then ["chong mat","sot"] -> 3 unique',
    a2keys.length === 3,
  ...
  test(
    'A3: addCluster same key twice -> no error, same cluster id',
    c1.id === c2.id,
    `id1=${c1.id}, id2=${c2.id}`
  );
  ...
  const { rows: a4 } = await pool.query(`SELECT * FROM fallback_logs WHERE user_id = $1`, [
    USER_ID,
  ]);
  test('A4: logFallback same symptom 5 times -> 5 rows', a4.length === 5, `rows=${a4.length}`);
  ...
  test(
    'A5: getUserScript 3 times -> same result (idempotent)',
  ...
  test(
    'A8: Count scripts after duplicates -> no orphans',
...
async function sectionB() {
  ...
  test(
    'B1: Symptom with special chars "dau dau (migraine)" -> cluster created',
  ...
  test(
    'B2: "DAU DAU" maps to same key as "dau dau"',
    keyUpper === keyLower,
  ...
  test(
    'B3: Extra spaces "  dau  dau  " -> handled (no crash)',
  ...
  test(
    'B4: Symptom with numbers "dau dau 3 ngay" -> cluster created',
  ...
  test(
    'B5: matchCluster "chong mat" vs stored cluster -> test behavior',
...
async function sectionC() {
  ...
  test(
    'C1: Create cluster + immediately getScript -> available',
  ...
  test(
    'C2: Two clusters in sequence -> both have scripts',
    c2a !== null && c2b !== null,
  ...
  const { rows: c3sessions } = await pool.query(
    `SELECT * FROM script_sessions WHERE user_id = $1 ORDER BY created_at`,
    [USER_ID]
  );
  test(
    'C3: Two sessions same day -> both exist in DB',
    c3sessions.length === 2,
  ...
  const { rows: c4 } = await pool.query(`SELECT * FROM fallback_logs WHERE user_id = $1`, [
    USER_ID,
  ]);
  test('C4: Multiple fallback logs same day -> all saved', c4.length === 3, `rows=${c4.length}`);
  ...
  test(
    'C5: getUserScript after 10 clusters -> returns all 10',
...
async function sectionD() {
  ...
  test(
    'D1: getNextQuestion with empty questions -> isDone immediately',
    d1.isDone === true,
    `isDone=${d1.isDone}`
  );
  ...
  test(
    'D2: evaluateScript with empty scoring_rules -> severity present',
  ...
  test(
    'D5: matchCluster for user_id=9999 -> matched=false, no crash',
    d5ok,
  ...
  test(
    'D6: getFallbackScriptData -> returns valid script',
    !!d6valid,
  ...
  const minScript = {
    questions: [{ id: 'q1', text: 'Test?', type: 'free_text' }],
  ...
  test(
    'D8: validateScript with empty object -> fails with errors',
  ...
  test(
    'D9: Slider answer 0 (falsy) -> scored correctly as low (0 < 4)',
  ...
  const d11 = getNextQuestion(skipScript, [{ question_id: 'q1', answer: 'test' }], {
    sessionType: 'initial',
    profile: {},
  });
  test(
    'D11: skip_if referencing non-existent field -> not triggered (q2 shown)',
  ...
  const oneQScript = {
    questions: [{ id: 'q1', text: 'Only question?', type: 'free_text' }],
    scoring_rules: [
  ...
  test(
    'D12a: Script with 1 question -> first call returns question',
    d12q.isDone === false && d12q.question?.id === 'q1',
    `isDone=${d12q.isDone}`
  );
  const d12done = getNextQuestion(oneQScript, [{ question_id: 'q1', answer: 'yes' }], {
    sessionType: 'initial',
    profile: {},
  });
  test(
    'D12b: After 1 answer -> concludes',
...
function sectionE() {
  ...
  test(
    'E1: "dau nguc" alone -> detected (high risk patient)',
    e1.severity !== 'low',
  ...
  test(
    'E6: "het kho tho roi" (past negation) -> NOT emergency',
  ...
  test(
    'E7: "dau nguc nhe" -> check behavior',
  ...
  const longText =
    'hom nay toi cam thay hoi met, dau dau nhe, an uong binh thuong, ' +
  ...
  test(
    'E10: Long text with "co giat" buried -> still detected',
...
async function sectionF() {
  ...
  test('F1: Create 5 clusters -> 5 rows in problem_clusters', f1.length === 5, `rows=${f1.length}`);
  ...
  const { rows: f2 } = await pool.query(
    `SELECT cluster_key, script_type FROM triage_scripts
     ...
     ORDER BY cluster_key, script_type`,
    [USER_ID]
  );
  const f2map = {};
  for (const row of f2) {
  ...
  const allHaveTwo = Object.values(f2map).every(
    (types) => types.length === 2 && types.includes('initial') && types.includes('followup')
  );
  test(
    'F2: Each cluster has exactly 2 scripts (initial + followup)',
    allHaveTwo,
  ...
  const { rows: f3 } = await pool.query(
    `SELECT * FROM fallback_logs WHERE user_id = $1 AND status = 'pending'`,
    [USER_ID]
  );
  test(
    'F3: Log 3 fallbacks -> 3 rows with status=pending',
    f3.length === 3,
    `pending=${f3.length}`
  );
  ...
  test('F4: Mark 1 as processed -> pending count = 2', f4.length === 2, `pending=${f4.length}`);

  // F5: Deactivate cluster -> scripts still exist but cluster inactive
  const clusterToDeactivate = f1[0];
  await pool.query(`UPDATE problem_clusters SET is_active = FALSE WHERE id = $1`, [
    clusterToDeactivate.id,
  ]);
  const { rows: f5cluster } = await pool.query(`SELECT * FROM problem_clusters WHERE id = $1`, [
    clusterToDeactivate.id,
  ]);
  const { rows: f5scripts } = await pool.query(
    `SELECT * FROM triage_scripts WHERE cluster_id = $1`,
    [clusterToDeactivate.id]
  );
  test(
    'F5: Deactivate cluster -> cluster inactive, scripts still exist',
